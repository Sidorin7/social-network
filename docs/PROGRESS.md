...
- [ ] Не начато (лайки/комментарии/подписки). В карточке поста уже есть визуальные заглушки под лайки/комментарии/репосты/просмотры (`PostCard`) и правая колонка под рекомендации — ждут реальных данных
